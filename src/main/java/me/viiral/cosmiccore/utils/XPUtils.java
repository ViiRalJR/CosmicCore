...
public class XPUtils {
    ...
    private static int getExpUntilNextLevel(final Player player) {
        int exp = Math.round(getExpAtLevel(player) * player.getExp());
        int nextLevel = player.getLevel();
        return getExpAtLevel(nextLevel) - exp;
    }

    public static boolean hasEnoughExperience(final Player player, final int amount) {
        return getTotalExperience(player) >= amount;
    }

    public static void removeExperience(final Player player, final int amount) {
        setTotalExperience(player, getTotalExperience(player) - amount);
    }

    public static void addExperience(final Player player, final int amount) {
        setTotalExperience(player, getTotalExperience(player) + amount);
    }
}
