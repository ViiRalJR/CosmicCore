...
public class Curse extends ArmorIncomingPVPDamageEventEnchant {
    ...
    public void runIncomingDamageEvent(EntityDamageByEntityEvent event, Player victim, LivingEntity attacker, EnchantInfo enchantInfo) {
        ...
        if (Math.random() < procChance) {
            if (victim.hasPotionEffect(PotionEffectType.INCREASE_DAMAGE))
                victim.addPotionEffect(new PotionEffect(PotionEffectType.INCREASE_DAMAGE, 30 * enchantInfo.getLevel(), 1));
            if (!victim.hasPotionEffect(PotionEffectType.DAMAGE_RESISTANCE))
                victim.addPotionEffect(new PotionEffect(PotionEffectType.DAMAGE_RESISTANCE, 30 * enchantInfo.getLevel(), 1));
